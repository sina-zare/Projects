...
with open("C:/restart.txt", 'r') as file:
    # Initialize an empty list to store the lines
    list_file = []
    # Loop through each line in the file
    for line in file:
        # Append the line to the list
        list_file.append(line.strip().lower())


for server in list_file:

    hostname = server + '.cloud.local'
    ...
    log_name = f"{hostname.split('.')[0]}-Fault-{current_time}-Log.txt"
