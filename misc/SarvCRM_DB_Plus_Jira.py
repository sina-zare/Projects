...
info_keywords = [ 'تمدی', 'تمدید', 'فاکتور',
                 'درخواست ساخت ماشین ابری', 'درخواست بروزرسانی ماشین ابری',
                 'درخواست قطع موقت ماشین ابری', 'درخواست وصل ماشین ابری','درخواست قطع دائم ماشین ابری', 'درخواست حذف ماشین ابری' ]
...
class Customer:
    ...
    def get_comments_rev(self, ticket_id):
        t_comments = self.tickets[ticket_id].comments

        reversed_comments = list(reversed(t_comments))
        ht_comments_list = []
...
{comment_value}
                '''
            ht_comments_list.append(ht_comment)

        return ht_comments_list
...
flag = 0
for customer_national_id in valid_customers:

    #if customer_national_id == '14009881433':

    if flag >= 5:
        print("Flag Exceeded 5 Outer")
        break

    for tkt in customers[customer_national_id].tickets:
        if flag >= 5:
            print("Flag Exceeded 5 Inner")
            break
        ...
        issue_dict = {
            ...
            'summary': f'{customers[customer_national_id].name}-{customers[customer_national_id].national_id}-{customers[customer_national_id].tickets[tkt].subject}-ArchiveTicket{customers[customer_national_id].tickets[tkt].ticket_id}',
            'customfield_11717': f'{customers[customer_national_id].tickets[tkt].subject}',  # Ticket Subject
            'customfield_11328': f'{customers[customer_national_id].name}',  # Customer Name
            'description': f'تاریخ ثبت تیکت: {customers[customer_national_id].tickets[tkt].date}\n\n{customers[customer_national_id].tickets[tkt].description}',
            'customfield_11223': f'{customers[customer_national_id].national_id}',  # National ID
        ...
        comments_for_each_ticket = customers[customer_national_id].get_comments_rev(tkt)

        for cmnt in comments_for_each_ticket:
            comment = jira.add_comment(new_issue, cmnt)
        ...
        print(f"Issue created: {new_issue.key} for {customers[customer_national_id].name} --> {customers[customer_national_id].tickets[tkt].ticket_id}")

        #flag += 1
        #print(f"flag incremented: {flag}")
