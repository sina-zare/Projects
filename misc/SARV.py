...
for i in valid_customers:
    if valid_customers[i].name:
        list_name.append(len(valid_customers[i].name))

    for t in valid_customers[i].tickets:

        list_subj.append(len(valid_customers[i].tickets[t].subject))

print(f'Maximum Customer Name Characters: {max(list_name)}')
print(f'Maximum Subject Characters: {max(list_subj)}')

print(f'Longest Phrase: {max(list_name) + max(list_subj) + 25}')
