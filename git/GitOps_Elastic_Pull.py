...
logging.info(f"=== Starting {SERVICE_NAME} config pull ===")
try:
    ensure_repo()
    '''
    Config file:
    ...
    '''
    try:
        copy_files([f"{REPO_PATH}/{SERVER_REPO_PATH}/elastic"], '/opt/compose-files/', preserve_tree=True)
        copy_files([f"{REPO_PATH}/{SERVER_REPO_PATH}/fluentd"], '/opt/compose-files/', preserve_tree=True)
        copy_files([f"{REPO_PATH}/{SERVER_REPO_PATH}/elast-alert"], '/opt/compose-files/', preserve_tree=True)
