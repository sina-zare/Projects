...
'''
Config file:
/opt/compose-files/elastic
/opt/compose-files/fluentd
/opt/compose-files/elast-alert
'''

# --- Configuration ---
SERVICE_NAME = 'ElasticSearch'
SERVER_NAME_FULL = (socket.gethostname()).lower()
SERVER_NAME_SUM = '-'.join((SERVER_NAME_FULL.split('-'))[:2]) # e.g. me-elastic
SERVER_REPO_PATH = f"{SERVER_NAME_SUM}/{SERVER_NAME_FULL}"
REPO_PATH = f"/opt/scripts/pull-push-config/git-files"
ES_FILES_TO_COPY = ["/opt/compose-files/elastic/*"]
FD_FILES_TO_COPY = ["/opt/compose-files/fluentd/*"]
EA_FILES_TO_COPY = ["/opt/compose-files/elast-alert/*"]




GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/elasticsearch.git"
BRANCH = "main"
...
def git_push():
    ...
    commit_message = f"{SERVICE_NAME} config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info(f"=== Starting {SERVICE_NAME} config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(ES_FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/elastic', preserve_tree=True)
    copy_files(FD_FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/fluentd', preserve_tree=True)
    copy_files(EA_FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/elast-alert', preserve_tree=True)
    git_push()
