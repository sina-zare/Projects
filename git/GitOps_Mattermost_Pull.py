...
GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/mattermost.git"
BRANCH = "main"
...
logging.info("=== Starting Mattermost config pull ===")
try:
    ensure_repo()
    '''
    Compose files:
    /opt/docker/docker-compose.nginx.yml
    ...
    Config file:
    /opt/docker/volumes/app/mattermost/config/.env
    '''
    try:
        copy_files([f"{REPO_PATH}/docker-compose*", f"{REPO_PATH}/.env"], '/opt/docker/')
        copy_files(f"{REPO_PATH}/.env", '/opt/docker/volumes/app/mattermost/config/')
