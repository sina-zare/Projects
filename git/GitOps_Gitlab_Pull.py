...
GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/gitlab.git"
BRANCH = "main"
...
logging.info("=== Starting GitLab config pull ===")
try:
    ensure_repo()
    '''
    Compose file:
    /opt/compose-files/gitlab/docker-compose.yml

    Config file:
    /opt/data/gitlab_data/config/gitlab.rb
    '''
    try:
        copy_files([f"{REPO_PATH}/vop-gitlab/docker-compose.yml"], '/opt/compose-files/gitlab')
        copy_files([f"{REPO_PATH}/vop-gitlab/gitlab.rb"], '/opt/data/gitlab_data/config')
