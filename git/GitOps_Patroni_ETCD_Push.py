...
PATRONI_FILES_TO_COPY = [
    "/etc/patroni/*",
]

GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/patroni.git"
BRANCH = "main"
...
def git_push():
    ...
    commit_message = f"Patroni-ETCD config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info("=== Starting Patroni and ETCD config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(ETCD_FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/etcd')
    copy_files(PATRONI_FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/patroni')
    git_push()
