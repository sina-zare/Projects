...
'''
    Config file:
    /opt/compose-files/kibana
'''

# --- Configuration ---
SERVICE_NAME = 'Kibana'
SERVER_NAME_FULL = (socket.gethostname()).lower() # me-kibana
#SERVER_NAME_SUM = '-'.join((SERVER_NAME_FULL.split('-'))[:2]) # e.g. "me-zabbix"
SERVER_REPO_PATH = f"{SERVER_NAME_FULL}"
...
try:
    ...
    git_pull()
    copy_files(FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}/', preserve_tree=True)
    git_push()
