...
GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/etcd.git"
BRANCH = "main"
...
def git_push():
    ...
    commit_message = f"ETCD config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info("=== Starting ETCD config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(FILES_TO_COPY, f'{REPO_PATH}/{SERVER_REPO_PATH}')
    git_push()
