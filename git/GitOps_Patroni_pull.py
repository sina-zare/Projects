...
def find_server_dir():
    """Return the path to the configs directory for this server."""
    server_directory = os.path.join(REPO_PATH, SERVER_REPO_PATH)
...
logging.info("=== Starting Patroni config pull ===")
try:
    ensure_repo()
    server_dir = find_server_dir()
    if not server_dir:
        raise FileNotFoundError(f"Server directory '{SERVER_REPO_PATH}' not found in repo '{REPO_PATH}'")

    logging.info(f"Found server dir: {server_dir}")
    print(f">>> Found server dir: {server_dir}")

    try:
        copy_files(f"{REPO_PATH}/{SERVER_REPO_PATH}/*", ETC_PATRONI)
        logging.info("✅ Configs applied successfully")
