...
def git_push():
    ...
    commit_message = f"Dashy config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info("=== Starting Dashy config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(COMPOSE_FILES_TO_COPY, f'{REPO_PATH}')
    copy_files(CONFIG_FILES_TO_COPY, f'{REPO_PATH}/user-data')
    copy_files(IMAGE_FILES_TO_COPY, f'{REPO_PATH}/user-data/item-icons')
    git_push()
