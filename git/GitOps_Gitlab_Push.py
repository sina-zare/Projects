...
def git_push():
    ...
    commit_message = f"GitLab config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info("=== Starting GitLab config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(FILES_TO_COPY, f'{REPO_PATH}/vop-gitlab')
    git_push()
