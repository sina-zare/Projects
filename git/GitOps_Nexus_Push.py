...
NEXUS_FILES_TO_COPY = [
    "/opt/compose-files/nexus/docker-compose.yml",
    "/opt/compose-files/nexus/nexus.properties",
]

HAPROXY_FILES_TO_COPY = [
    "/etc/haproxy/haproxy.cfg",
]
GITLAB_URL = "https://gitlab.abramad.com/abramad/sysops/nexus.git"
BRANCH = "main"
...
def git_push():
    ...
    commit_message = f"Nexus config update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
try:
    logging.info("=== Starting Nexus config push ===")
    os.chdir(REPO_PATH)
    setup_git_identity()   # <-- set Git identity locally
    git_pull()
    copy_files(NEXUS_FILES_TO_COPY, f'{REPO_PATH}/nexus')
    copy_files(HAPROXY_FILES_TO_COPY, f'{REPO_PATH}/haproxy')
    git_push()
