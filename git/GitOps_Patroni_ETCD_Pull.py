...
logging.info("=== Starting Patroni and ETCD config pull ===")
try:
    ensure_repo()
    '''
    Config file:
    ...
    '''
    try:
        copy_files([f"{REPO_PATH}/{SERVER_REPO_PATH}/patroni/*"], '/etc/patroni')
        copy_files([f"{REPO_PATH}/{SERVER_REPO_PATH}/etcd/etcd"], '/etc/default')
