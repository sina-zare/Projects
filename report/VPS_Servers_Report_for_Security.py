...
username = decryptor("enc_sinaz_sgcloud","key_sinaz_sgcloud")
...
current_month = f'VPS-Security-Report-{current_persian_month}-1402'
...
me_vms = [vm for vm in me_vm_view.view if ( vm.name.startswith("VPS-") or not vm.name.lower().startswith("VPS-") )]
sorted_vms = sorted(me_vms, key=lambda vm: vm.name.lower())

# Specify the filename and full path to the CSV file
vps_csv_file_path = f'VPS-Customers-Server-Report-{current_persian_month}-1402.csv'
vps_sec_report_full_file_path = os.path.join(final_path, vps_csv_file_path)

with open(vps_sec_report_full_file_path, mode='w', newline='', encoding='utf-8') as file:
...
# Count number of servers per product and take their Persian name from custom attributes
    for vm in sorted_vms:
        ...
        if (vm.name.lower().startswith("vps-")):
...
# Specify the filename and full path to the CSV file
MGMT_csv_file_path = f'VPS-Management-Servers-Report-{current_persian_month}-1402.csv'
...
# Count number of servers per product and take their Persian name from custom attributes
    for vm in sorted_vms:
        # Check count of Rahkaran servers
        if (not vm.name.lower().startswith("vps-")):
...
email_body = html_msg_1 + html_msg_2 + html_msg_3 + html_line_break + html_msg_4 + html_line_break + html_msg_5 + html_msg_6
