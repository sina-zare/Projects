...
1) Take all valid RA-* vms and their Tags from its vCenter
2) Take all valid RA-* vms and their RAM, CPU, Disk from its vCenter
3) Join all step 1&2 and outputs a file accourding to the given month

4) Take all valid VPS-* vms and their Tags from its vCenter
5) Take all valid VPS-* vms and their RAM, CPU, Disk from its vCenter
6) Join all step 4&5 and outputs a file accourding to the given month
...
sgc_csv_tag_filename = f'VPS-Tag-{current_month}.csv'
sgc_csv_tag_path = os.path.join(final_path, sgc_csv_tag_filename)
...
with open(sgc_csv_rep_path, mode='w', newline='', encoding='utf-8') as file:
    ...
    # Write the header row to the CSV file
    writer.writerow(['VM Name', 'VM Power State', 'RAM (GB)', 'CPU', 'Disk (GB)', 'VM Tag', 'VM MoU & Farsi Name'])
