...
me_vms = [vm for vm in me_vm_view.view if (vm.name.lower().startswith("mev-"))]
...
for vm in sorted_vms:
    ...
    is_pure_daas = ''
    custom_value_type = vm.summary.customValue
    for v in custom_value_type:
        if v.key == 306:
            is_pure_daas = v.value

    is_sepidar_daas = ''
    custom_value_type = vm.summary.customValue
    for v in custom_value_type:
        if v.key == 307:
            is_sepidar_daas = v.value
    ...
    if is_pure_daas == '1':
        c_type = 'Pure DaaS'
    elif is_sepidar_daas == '1':
        c_type = 'Sepidar DaaS'
    elif is_sedaas == '1':
        c_type = 'SeDaaS'

    list.append([vm.name, nat_id, p_name, c_type])
...
# Write the dictionary data to the worksheet
header = ["VM Name", "National_ID", "Persian Name", "Type"]
worksheet.append(header)


for vm_item in list:
    worksheet.append(vm_item)

# Save the changes to the Excel file
workbook.save('C:/Users/sina.z/Desktop/Automation_Reports/DaaS_Report/DaaS.xlsx')
