...
def init_restic_repo():
    ...
    #    cprint("info", Fore.LIGHTGREEN_EX, "✅ Repository initialized.")
        ...
        cprint("warning", Fore.LIGHTYELLOW_EX, "Repository not found. Initializing...")
        try:
            run_command(["restic", "init"], check=True)
            cprint("info", Fore.LIGHTGREEN_EX, "✅ Repository initialized.")
        except subprocess.CalledProcessError as e:
            cprint("error", Fore.LIGHTRED_EX, f"❌ restic init failed: {e}")
            # raise so outer code will mark success=False
            raise
...
def pgdump_backup_remote_streamed(remote_host_name, remote_host_ip):
    ...
    # capture stderr so we can include it in the exception
    p3 = subprocess.Popen(restic_cmd, stdin=p2.stdout, stderr=subprocess.PIPE, text=True)
    p1.stdout.close()
    p2.stdout.close()
    p3_ret = p3.wait()
    stderr = None
    try:
        if p3.stderr:
            stderr = p3.stderr.read().strip()
    except Exception:
        stderr = None

    if p3_ret != 0:
        cprint("error", Fore.LIGHTRED_EX, "❌ Restic failed to back up the stream.")
        raise RuntimeError(f"restic backup failed (exit {p3_ret}) - {stderr or 'no stderr'}")
...
def run_backup_dir(remote_host_name, remote_host_ip, remote_dir):
    ...
    cprint("info", Fore.LIGHTGREEN_EX,
           f"✅ Directory backup of {remote_dir} from {remote_host_name} completed successfully.")
...
def restore_backup(remote_host_name, snapshot_id="latest"):
    ...
    if host_restore_display:
        cprint("info", Fore.LIGHTGREEN_EX,
               f"✅ Backup restore completed into: {restic_restore_dir} (host: {host_restore_display})")
    else:
...
if len(sys.argv) < 2:
    cprint("error", Fore.LIGHTRED_EX,
           "❌ Expecting at least one argument:" + Fore.LIGHTWHITE_EX + "[backup|restore|remove|list]")
    raise SystemExit(1)
...
with open(restic_client_file, "r", encoding="utf-8") as file:
    ...
    for line in lines:
        line = line.strip()
        if not line or line == '_Hostname_:_Host_IP_:_Restic_User_:_Restic_Password_:_Repo_Password_:_Backup_Type_:_Backup_Dir_':
            continue  # Skip empty/header lines if present
        tmp_list = line.split(':')
        if len(tmp_list) < 6:
            cprint("warning", Fore.LIGHTMAGENTA_EX,
...
if action == "backup":  # we set metric values here
    if target == 'all':
        for hostname, hostdata in restic_clients.items():
            ...
            try:
                ...
                if hostdata['backup_type'] == 'psql':
                    backup_success.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                          backup_dir='PostgreSQL dump').set(0)
                    backup_duration.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                           backup_dir='PostgreSQL dump').set(0.0)

                    cprint("info", Fore.LIGHTWHITE_EX,
                           f"Starting backup from {hostdata['host_name']}:{hostdata['backup_type']}\n")
                    init_restic_repo()
                    ...
                    backup_success.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                          backup_dir=hostdata['backup_dir']).set(0)
                    backup_duration.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                           backup_dir=hostdata['backup_dir']).set(0.0)

                    cprint("info", Fore.LIGHTWHITE_EX,
                           f"Starting backup from {hostdata['host_name']}:{hostdata['backup_dir']}\n")
                    init_restic_repo()
                ...
                if hostdata['backup_type'] == 'psql':
                    backup_success.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                          backup_dir='PostgreSQL dump').set(1 if success else 0)
                    backup_duration.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                           backup_dir='PostgreSQL dump').set(duration)
                elif hostdata['backup_type'] == 'dir':
                    backup_success.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                          backup_dir=hostdata['backup_dir']).set(1 if success else 0)
                    backup_duration.labels(client=hostdata['host_name'], backup_type=hostdata['backup_type'],
                                           backup_dir=hostdata['backup_dir']).set(duration)
        try:
            ...
            if pushgateway_url:
                ...
                cprint("warning", Fore.LIGHTYELLOW_EX,
                       f"No pushgateway env provided in .env to push metrics (PUSHGATEWAY_URL='{pushgateway_url}')")
                cprint("info", Fore.LIGHTYELLOW_EX, (121 * '#') + '\n')
        ...
        for i, host in enumerate(restic_clients.values(), 1):
            ...
            backup_type = Fore.YELLOW + host['backup_type'] + Style.RESET_ALL if host[
                                                                                     'backup_type'] == 'dir' else Fore.GREEN + \
                                                                                                                  host[
                                                                                                                      'backup_type'] + Style.RESET_ALL
            backup_dir = Fore.MAGENTA + (
                "PostgreSQL dump" if host['backup_type'] == 'psql' else host['backup_dir']) + Style.RESET_ALL
            table_data.append([i, hostname, backup_type, backup_dir])

        cprint("info", Fore.LIGHTWHITE_EX, "Select one host ID to backup:\n")
        print(tabulate(table_data, headers=["ID", "Hostname", "Backup Type", "Backup Dir/File"],
        ...
        if selected_host:
            ...
            try:
                ...
                if selected_host['backup_type'] == 'psql':
                    backup_success.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                          backup_dir='PostgreSQL dump').set(0)
                    backup_duration.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                           backup_dir='PostgreSQL dump').set(0.0)

                    cprint("info", Fore.LIGHTWHITE_EX,
                           f"Starting backup from {selected_host['host_name']}:{selected_host['backup_type']}\n")
                    init_restic_repo()
                    run_backup_streamed(selected_host['host_name'], selected_host['host_ip'])

                elif selected_host['backup_type'] == 'dir':
                    backup_success.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                          backup_dir=selected_host['backup_dir']).set(0)
                    backup_duration.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                           backup_dir=selected_host['backup_dir']).set(0.0)

                    cprint("info", Fore.LIGHTWHITE_EX,
                           f"Starting backup from {selected_host['host_name']}:{selected_host['backup_dir']}\n")
                    init_restic_repo()
                ...
                if selected_host['backup_type'] == 'psql':
                    ...
                    backup_success.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                          backup_dir=selected_host['backup_dir']).set(1 if success else 0)
                    backup_duration.labels(client=selected_host['host_name'], backup_type=selected_host['backup_type'],
                                           backup_dir=selected_host['backup_dir']).set(duration)
    ...
    if target == 'all':
        ...
        if selected_host:
            input_snapshot = (
                input(Fore.LIGHTWHITE_EX + 'Enter snapshot ID to remove[ID|latest|all]:(latest) ')).strip()
            restic_repo = f"rest:http://{selected_host['restic_user']}:{selected_host['restic_pass']}@{rest_server_host}:{rest_server_port}/{selected_host['restic_user']}"
            os.environ["RESTIC_REPOSITORY"] = restic_repo
            os.environ["RESTIC_PASSWORD"] = selected_host.get('repo_pass', '')
