...
def pgdump_backup_remote_streamed(remote_host_name, remote_host_ip):
    print(f"📡 Starting streamed backup from {remote_host_name}...")
    ...
    p3_ret = p3.wait()
    if p3_ret != 0:
        print("❌ Restic failed to back up the stream.")
        sys.exit(1)
    print("✅ Streamed backup completed.")
...
restic_dump_dir = "/opt/s3-mount/restic-backup-restore/dump"
