try:
    import sys
    sys.path.append(r'D:\IaaC') # to use sinzi
    import sinzi
    ...
    log_path = f"D:\\IaaC\\Block_IPs\\Logs\\{today_date_jalali}"
    ...
    for device in fg_devices:
        ...
        # Appending BlackListIPs Groups to Substantial BlackListIPsGroup
        ...
        append_giantgroup_commands = sinzi.append_addresses_to_group(f"BlackListIPsGroup", black_ips_list, "24")
        append_giantgroup_config = ssh_to_device.send_config_set(append_giantgroup_commands)
        sinzi.check_successful_execution_g(append_giantgroup_config,
                                           f"Append {len(black_ips_list)} Groups to Giant BlackListIPsGroup Group",
                                           "*)", f"BlackListIPs{latest_number}", full_log_path)
