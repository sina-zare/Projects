...
old_cert_name = 'Abramad.cloud-2024-2025'
new_cert_name = 'CertumFull.Abrmad.Cloud-2025-2026'
    ...
    if line.startswith('edit "'):
        ...
        if (idx + 1) < len(show_all_vips_list):
            ...
            if line == 'next' and (show_all_vips_list[idx + 1]).strip().startswith('edit "'):
                ...
                if f'set ssl-certificate "{old_cert_name}"' in block_text:
...
if f'set ssl-certificate "{old_cert_name}"' in show_all_vips_list[-3]:
...
#print('_2024_2025_waf_members:')
#for i in _2024_2025_waf_members:
#    print(i)
#print(len(_2024_2025_waf_members))
    ...
    if line.startswith('edit "'):
        ...
        if (idx + 1) < len(show_all_vips_list):
            ...
            if line == 'next' and (show_all_vips_list[idx + 1]).strip().startswith('edit "'):
                ...
                #print(block_text)
                # Check if the block contains the desired SSL certificate
                if f'set ssl-certificate "{new_cert_name}"' in block_text:
...
# handling last block of config:
if f'set ssl-certificate "{new_cert_name}"' in show_all_vips_list[-3]:
...
for node in _2024_2025_waf_members[:]:
    #if not 'mahya' in node.lower():
    print(f"Working on: {node}")
    # Getting all firewall groups to take out last BlackListIPs
    set_certificate_commands = ['config firewall vip', f'edit {node}', f'set ssl-certificate "{new_cert_name}"', 'end']
