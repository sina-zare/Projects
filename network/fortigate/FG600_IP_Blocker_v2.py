try:
    ...
    if len(ips_to_block_list) > 600:
        with open(f"{log_path}\\{current_time}-FG600-BlockIP-Log.txt", "a") as text_file:
            text_file.write(
    ...
    for device in fg_devices:
        ...
        sinzi.check_successful_execution_g(append_giantgroup_config, f"Append {len(black_ips_list)} Groups to Giant BlackListIPsGroup Group","*)", f"BlackListIPs{latest_number}", full_log_path)
