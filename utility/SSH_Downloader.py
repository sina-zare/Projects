...
def find_remote_paths(ssh_client, remote_prefix):
    stdin, stdout, stderr = ssh_client.exec_command(f"find {remote_prefix}*")
...
print("\n\n")
remote_prefix = f'/var/spool/asterisk/monitor/2024/{date}/external-{phone_id}'
local_base_path = f'C:/Temp/{date_path}-ID-{phone_id}'

ssh_client = create_ssh_client(server, port, user, password)
try:
    if not os.path.exists(local_base_path):
        os.makedirs(local_base_path)
    remote_paths = find_remote_paths(ssh_client, remote_prefix)
    if not remote_paths:
