try:
    ...
    def send_anonymous_email(from_email, to_email, cc_email, subject, html_message, direction,
        ...
        try:
            ...
            send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                 ...
                                 f"Error Occurred:<br><b>{err}<br>{traceback.print_exc()}</b> Agent: haproxy-config-creator.py",
                                 'ltr')
   ...
   # Date Info
    ...
    me_vms = [vm for vm in me_vm_view.view if ( (vm.name.startswith("VRA-")) or (vm.name.startswith("VRT-")) or (vm.name.startswith("VRF-")) ) and not vm.name.startswith("VRA-HAProxy") and not vm.name.startswith("VRA-Template")]
    ...
    for vm in sorted_vms:
        ...
        if vm_power_state.lower() == "poweredon":
            ...
            for device in vm.config.hardware.device:
                if isinstance(device, vim.vm.device.VirtualEthernetCard):
                    ...
                    if isinstance(network, vim.vm.device.VirtualEthernetCard.DistributedVirtualPortBackingInfo):
                        ...
                        dvs = me_content.dvSwitchManager.QueryDvsByUuid(dvs_uuid)
            ...
            if vm_portgroup.startswith('RA-Customers-N1003-VM'):
...
frontend http_front
    ...
    acl {(ra_dict[vm][0][4:]).lower()}_acl hdr(host) -i -m reg ^{((ra_dict[vm][0])[4:]).lower()}\.rahkaran\.ir$
...
"""
                ...
                send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                     f"Name Resolution unsuccessful on {vm} | {hostname}",
                                     f"Error Occurred:<br><b>{today_date_jalali_full}<br><b>{vm}</b> was not Resolvable<br>Rolling back the configuration<br>HAProxy service left untouched.<br><br>{traceback.print_exc()}</b> Agent: haproxy-config-creator.py",
                                     'ltr')
        ...
        try:
            ...
            mail_body = f"{today_date_jalali_full}<br>Number of VMs in cache file was not equal to vCenter VMs  --> <b>vCenter: {ra_servers_count}  !=  File: {ra_vms_count_file}</b><br>Generating New 'haproxy' config<br>HAProxy service reloaded successfully.<br><br>Agent: haproxy-config-creator.py"
            send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                 f"New HAProxy Config Creation Succeeded | {hostname}",
                                 mail_body,'ltr')
            ...
            send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                 f"Failure in Reloading HAProxy Service | {hostname}",
                                 f"<b>Critical Error!<br>Check {hostname} HAProxy Logs ASAP.</b><br><br>Agent: haproxy-config-creator.py", 'ltr')
...
"""
                        ...
                        send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                             f"Name Resolution unsuccessful on {vm} | {hostname}",
                                             f"{today_date_jalali_full}<br><b>{vm}</b> was not Resolvable<br>Rolling back the configuration<br>HAProxy service left untouched.<br><br>Agent: haproxy-config-creator.py",
                                             'ltr')
                ...
                try:
                    ...
                    mail_body = f"{today_date_jalali_full}<br>Number of VMs in cache file was equal to vCenter VMs, but VM names were not the same.<br><b>{virtual_machine}</b> was not in cache file<br>Generating New 'haproxy' config<br>HAProxy service reloaded successfully.<br><br>Agent: haproxy-config-creator.py"
                    send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                                         f"New HAProxy Config Creation Succeeded | {hostname}",
                                         mail_body, 'ltr')
    ...
    send_anonymous_email('VRA-HAProxy@abramad.com', default_receivers, default_cc,
                         f"General Error on running config-creator.py | {hostname}",
                         f"Error Occurred:<br><b>{err}</b><br><br>Agent: haproxy-config-creator.py",
                         'ltr')
