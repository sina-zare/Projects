try:
    ...
    reci = "support@abramad.com,mehdi.a@abramad.com,alireza.ja@abramad.com"
    ...
    ME_VC = connect.SmartConnect(host='mra-vc01.abramad.com', user=username, pwd=password, port=443, sslContext=context)
    ...
    me_vms = [vm for vm in me_vm_view.view if (vm.name.startswith("RA-"))]
...
frontend https_front
    ...
    acl {(ra_dict[vm][0][3:]).lower()}_acl hdr(host) -i -m reg ^{((ra_dict[vm][0])[3:]).lower()}\.servehttp\.com$
    use_backend {(ra_dict[vm][0])} if {(ra_dict[vm][0][3:]).lower()}_acl
