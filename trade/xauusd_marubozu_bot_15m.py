try:
    ...
    import pandas as pd
    from sinzi import is_marubozu, convert_miladi_to_shamsi, sinzi_logo
    import time
    import jdatetime
    import asyncio
    ...
    # Symbol Info
    log_path = "xauusd_marubozu_15m.log"
    image_path = "xauusd_marubozu_15m.png"
    symbol = "XAUUSD"
    timeframe = mt5.TIMEFRAME_M15
    sleep_time = int(timeframe) * 60

    # Initialize Telegram Bot
    bot = Bot(token='7437344829:AAE6LdvFphkdqAJrH2qjmXUUbFRofBe12RU')
                ...
                if is_marubozu(last_candle):
                    ...
                    if bars is not None and bars.size > 0:
                        ...
                        if os.path.exists(image_path) and os.path.getsize(image_path) > 0:
                            ...
                            marubozu_body = f"{today_date_jalali_full}\n\nSymbol: {symbol}\nTime Frame: {timeframe} Min\nMarubozu Pattern Formed at {shamsi_time}\n\n~~~~ ~~~~ ~~~~~ ~~~~ ~~~~ "

                            print(10 * '~' + f'{symbol} {timeframe}M' + 10 * '~')
                            with open(log_path, "a") as log:
                                log.write(10 * '~' + f'{symbol} {timeframe}M' + 10 * '~' + "\n")
                            print(f'marubozu at {shamsi_date} {shamsi_time}\n')
                            with open(log_path, "a") as log:
                                log.write(f'marubozu at {shamsi_date} {shamsi_time}\n' + "\n")
                            ...
                            for userid in chat_ids:
                                ...
                                await bot.send_message(chat_id=userid[0], text=marubozu_body)
