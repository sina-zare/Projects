...
try:
    ...
    with open("C:\\Temp\\temp-vra.txt", "w", encoding="utf-8") as file:
        for i in monitor_list:
            file.write(f"{i[0]},{i[1]},{i[2]},{i[3]}\n")

    for node in monitor_list:
        api.delete_monitor(int(node[0]))
        print(f"{node[1]} got deleted")
        time.sleep(0.2)
    ...
    monitor_list2 = []
    with open("C:\\Temp\\temp-vra.txt", "r", encoding="utf-8") as file:
        for i in file:
            monitor_list2.append(i.replace('\n', '').split(','))


    
    api02 = UptimeKumaApi('http://172.17.255.85:3015/')
