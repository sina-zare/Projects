...
# --- Configuration ---
script_name = 'zabbix_agent_manager_vnk_fully_automated'
...
datacenter = 'vanak'
target = 'vnk_onprem_abri_customers'
...
try:
    ...
    zabbix_servers = [
            ...
            {
                'zabbix_address': 'http://vnk-customerzabbix.abramad.com/',
                'vcenter_abri_address': 'vra-vc01.abramad.com',
                'vcenter_onprem_address': 'vab-vc01.abramad.com',
                'from_address': 'vnk-customerzabbix@abramad.com'
            },
    ...
    try:
        ...
        for iteration in zabbix_servers:
            ...
            vc_me_vms = [vm for vm in vc_vm_view.view if ( (vm.name.lower().startswith("vra-")) or (vm.name.lower().startswith("vrf-")) or (vm.name.lower().startswith("vrt-"))) and ('template' not in vm.name.lower()) and ('vra-pentest' not in vm.name.lower())]
            ...
            for vm in sorted_abri_vms[:]:
                ...
                if vm_portgroup.startswith('RA-Customers-N1003-VM'): #and vm.name.lower() != 'vra-amini2' and vm.name.lower() != 'vra-amini3' and vm.name.lower() != 'vra-demo' and vm.name.lower() != 'vra-bi1' and not vm.name.lower().startswith('vra-haproxy'):
                    ...
                    #vra_disconnect_ticket_id = vm_attrs.get('vm_disconnect_ticket_no', 'Null')

                    #  Distinguishing VMs
                    vcenter_abri_vms[vm_name.lower()] = [vm_name]

                    if vm_power_state.endswith('off') and vra_shutdown_ticket_id != 'Null':  # if vm is shutdown and shutdown ticket is set
            ...
            vc_me_vms = [vm for vm in vc_vm_view.view if (
                        (vm.name.lower().startswith("vr1-")) or
                        (vm.name.lower().startswith("vr2-")) or
                        (vm.name.lower().startswith("vr3-")) or
                        (vm.name.lower().startswith("vrd-")) or
                        (vm.name.lower().startswith("vbi-")) or
                        (vm.name.lower().startswith("vsp-")) or
                        (vm.name.lower().startswith("via-")) or
                        (vm.name.lower().startswith("vmi-")) or
                        (vm.name.lower().startswith("vat-")) or
                        (vm.name.lower().startswith("vsf-"))
                        )
            ...
            for vm in sorted_onprem_vms[:]:
                ...
                # set vm custom attributes
                vab_shutdown_ticket_id = vm_attrs.get('vm_shutdown_ticket_no', 'Null')
                vab_disconnect_ticket_id = vm_attrs.get('vm_disconnect_ticket_no', 'Null')

                #  Distinguishing VMs
                vcenter_onprem_vms[vm_name.lower()] = [vm_name]

                if vm_power_state.endswith('off') and vab_shutdown_ticket_id != 'Null':  # if vm is shutdown and shutdown ticket is set
                    vcenter_onprem_poff_vms[vm_name.lower()] = [vm_name]

                elif vm_nic_status == 'disconnected' and vab_disconnect_ticket_id != 'Null':  # if vm is disconnected adn disconnect ticket is set
                    vcenter_onprem_poff_vms[vm_name.lower()] = [vm_name]

                elif vm_power_state.endswith('on') and vm_nic_status == 'connected':
                    vcenter_onprem_pon_vms[vm_name.lower()] = [vm_name]
            ...
            for host in hosts:
                ...
                if (host['host'].lower().startswith('vr1-') or
                    host['host'].lower().startswith('vr2-') or
                    host['host'].lower().startswith('vr3-') or
                    host['host'].lower().startswith('vrd-') or
                    host['host'].lower().startswith('vbi-') or
                    host['host'].lower().startswith('vsp-') or
                    host['host'].lower().startswith('via-') or
                    host['host'].lower().startswith('vmi-') or
                    host['host'].lower().startswith('vat-') or
                    host['host'].lower().startswith('vsf-')):

                    zabbix_onprem_vms[host['host'].lower()] = [host['host'].lower(), host['hostid'], host['status']]

                if ((host['host'].lower().startswith('vr1-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vr2-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vr3-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vrd-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vbi-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vsp-') and host['status'] == '1') or
                    (host['host'].lower().startswith('via-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vmi-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vat-') and host['status'] == '1') or
                    (host['host'].lower().startswith('vsf-') and host['status'] == '1') ):

                    zabbix_onprem_poff_vms[host['host'].lower()] = [host['host'].lower(), host['hostid'], host['status']]

                if ((host['host'].lower().startswith('vr1-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vr2-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vr3-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vrd-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vbi-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vsp-') and host['status'] == '0') or
                    (host['host'].lower().startswith('via-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vmi-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vat-') and host['status'] == '0') or
                    (host['host'].lower().startswith('vsf-') and host['status'] == '0') ):
                ...
                if zb_abri_node not in set(vcenter_abri_vms.keys()):  # Node Needs to be deleted
                    try:
                        ...
                        send_anonymous_email(
                            ...
                            cc_email=default_cc,
                            subject=f"Error deleting node '{zb_abri_node}' | Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            html_message=f"Error deleting node:<br><b>{zb_abri_node}: {node_error} {traceback.print_exc()}<br>Exception type: {type(node_error).__name__}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            direction="ltr",
                            # attachments=[]
                        )
                ...
                if zb_onprem_node not in set(vcenter_onprem_vms.keys()):  # Node Needs to be deleted
                    try:
                        ...
                        send_anonymous_email(
                            ...
                            to_email=error_receivers,
                            cc_email=default_cc,
                            subject=f"Error deleting node '{zb_onprem_node}' | Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            html_message=f"Error deleting node:<br><b>{zb_onprem_node}: {node_error} {traceback.print_exc()}<br>Exception type: {type(node_error).__name__}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            direction="ltr",
                            # attachments=[]
                        )
                ...
                if vc_pon in set(zabbix_abri_poff_vms.keys()):
                    try:
                        ...
                        send_anonymous_email(
                            ...
                            to_email=error_receivers,
                            cc_email=default_cc,
                            subject=f"Error enabling node '{vc_pon}' | Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            html_message=f"Error enabling node:<br><b>{vc_pon}: {node_error} {traceback.print_exc()}<br>Exception type: {type(node_error).__name__}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            direction="ltr",
                            # attachments=[]
                        )
                ...
                if vc_poff in set(zabbix_abri_pon_vms.keys()):  # Node Needs to be disabled
                    try:
                        ...
                        send_anonymous_email(
                            ...
                            to_email=error_receivers,
                            cc_email=default_cc,
                            subject=f"Error disabling node '{vc_poff}' | Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            html_message=f"Error disabling node:<br><b>{vc_poff}: {node_error} {traceback.print_exc()}<br>Exception type: {type(node_error).__name__}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                            direction="ltr",
                            # attachments=[]
                        )
            ...
            if len(zabbix_nodes_enabled) > 0:
                ...
                send_anonymous_email(
                    ...
                    html_message=f"Below nodes were enabled again in {from_address.split('@')[0]} automatically:<br><br><b>{temp}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                    direction="ltr",
                    # attachments=[]
                )
            ...
            if len(zabbix_nodes_disabled) > 0:
                ...
                send_anonymous_email(
                    ...
                    html_message=f"Below nodes were disabled in {from_address.split('@')[0]} automatically:<br><br><b>{temp}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                    direction="ltr",
                    # attachments=[]
                )
            ...
            if len(zabbix_nodes_deleted) > 0:
                ...
                send_anonymous_email(
                    ...
                    html_message=f"Below nodes were deleted from {from_address.split('@')[0]} automatically:<br><br><b>{temp}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
                    direction="ltr",
                    # attachments=[]
                )
        ...
        send_anonymous_email(
            ...
            to_email=error_receivers,
            cc_email=default_cc,
            subject=f"Body Error in running Zabbix_Agent_Manager_VNK_Fully_Automated.py",
            html_message=f"Error Occurred:<br><b>{body_error}<br>{traceback.print_exc()}<br>Exception type: {type(body_error).__name__}</b> Agent: Zabbix_Agent_Manager_VNK_Fully_Automated.py",
            direction="ltr",
            # attachments=[]
        )
