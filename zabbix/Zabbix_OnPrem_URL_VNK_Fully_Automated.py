try:
    ...
    try:
        ...
        def send_anonymous_email(from_email, to_email, cc_email, subject, html_message, direction,
            ...
            try:
                ...
                send_anonymous_email('ScriptError@abramad.com', error_receivers, 'sina.z@abramad.com',
                                     f"email_sender Function Error in running Zabbix_OnPrem_URL_VNK_Fully_Automated.py",
                                     f"Error Occurred:<br><b>{err}<br></b>Exception type: {type(err).__name__}<br> Agent: Zabbix_OnPrem_VNK_Fully_Automated.py",
                                     'ltr')
        ...
        send_anonymous_email(
            ...
            cc_email=default_cc,
            subject=f"Email Function Error in running Zabbix_OnPrem_VNK_Fully_Automated.py",
    ...
    try:
        ...
        me_vc = connect.SmartConnect(host='vab-vc01.abramad.com', user=username, pwd=passphrase, port=443, sslContext=context)
        ...
        me_vms = [vm for vm in me_vm_view.view if
                  (vm.name.lower().startswith("vr1-")) or (vm.name.lower().startswith("vr2-")) or (vm.name.lower().startswith("vr3-")) or (vm.name.lower().startswith("vat-"))]
        sorted_vms = sorted(me_vms, key=lambda vm: vm.name.lower())
        sorted_vms_fnl = []

        # Pruning VM list
        for vm in sorted_vms:
            if not vm.name.lower().endswith('-db') and not vm.name.lower().endswith('-a2') and not vm.name.lower().endswith('-a3') and not vm.name.lower().endswith('-a4') and not vm.name.lower().endswith('-a5') and not vm.name.lower().endswith('-a6'):
            ...
            try:
                ...
                if vm_power_state == 'poweredon':
                    ...
                    try:
                        ...
                        print(f"Calculation Error: {calc_err} on {vm_name}\nCreation Date: {vm_creation_date}")
                        print(f"Exception type: {type(calc_err).__name__}")
                        send_anonymous_email('ScriptError@abramad.com', error_receivers, 'sina.z@abramad.com',
                                             f"Calculation Error in running Zabbix_OnPrem_URL_VNK_Fully_Automated.py",
                                             f"Error Occurred:<br><br>{calc_err}<br>Exception type: {type(calc_err).__name__}<br><br>Description:<br><b>Calculation error occurred, Check the 'Creation Date' attribute in vCenter on {vm_name}</b><br><br>Agent: Zabbix_OnPrem_VNK_Fully_Automated.py",
                                             'ltr')
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Body Error in running Zabbix_OnPrem_VNK_Fully_Automated.py",
        ...
        for host in hosts:
            if host['host'].startswith('vr1-') or host['host'].startswith('vr2-') or host['host'].startswith('vr3-') or host['host'].startswith('vat-'):
        ...
        template_vr = "Rahkaran-OnPrem-Tpl-Far"  # Template name
        template_vat = "Automation-OnPrem-Tpl"  # Template name
        host_group_vr1 = "VR1"  # Host group name
        host_group_vr2 = "VR2"  # Host group name
        host_group_vr3 = "VR3"  # Host group name
        host_group_support = "Support_Team"  # Host group name
        host_group_vrvip = "Rahkaran_OnPrem_VIP"  # Host group name
        host_group_onprem = "Rahkaran_OnPrem_Grp"  # Host group name
        host_group_vat = "VAT"  # Host group name
        host_group_url = "Rahkaran-Abri-Different_URL"

        # Step 1: Get the template ID
        template1 = zapi.template.get(filter={"host": template_vr})
        if not template1:
            raise Exception(f"Template '{template_vr}' not found.")
        template_vr_id = template1[0]["templateid"]

        template2 = zapi.template.get(filter={"host": template_vat})
        if not template2:
            raise Exception(f"Template '{template_vat}' not found.")
        template_vat_id = template2[0]["templateid"]


        # Step 2: Get the host group ID
        host_group1 = zapi.hostgroup.get(filter={"name": host_group_vr1})
        if not host_group1:
            raise Exception(f"Host group '{host_group_vr1}' not found.")
        host_group_vr1_id = host_group1[0]["groupid"]

        host_group2 = zapi.hostgroup.get(filter={"name": host_group_vr2})
        if not host_group2:
            raise Exception(f"Host group '{host_group_vr2}' not found.")
        host_group_vr2_id = host_group2[0]["groupid"]

        host_group3 = zapi.hostgroup.get(filter={"name": host_group_vr3})
        if not host_group3:
            raise Exception(f"Host group '{host_group_vr3}' not found.")
        host_group_vr3_id = host_group3[0]["groupid"]

        host_group4 = zapi.hostgroup.get(filter={"name": host_group_support})
        if not host_group4:
            raise Exception(f"Host group '{host_group_support}' not found.")
        host_group_support_id = host_group4[0]["groupid"]

        host_group5 = zapi.hostgroup.get(filter={"name": host_group_vrvip})
        if not host_group5:
            raise Exception(f"Host group '{host_group_vrvip}' not found.")
        host_group_vrvip_id = host_group5[0]["groupid"]

        host_group6 = zapi.hostgroup.get(filter={"name": host_group_vat})
        if not host_group6:
            raise Exception(f"Host group '{host_group_vat}' not found.")
        host_group_vat_id = host_group6[0]["groupid"]

        host_group7 = zapi.hostgroup.get(filter={"name": host_group_onprem})
        if not host_group7:
            raise Exception(f"Host group '{host_group_onprem}' not found.")
        host_group_onprem_id = host_group7[0]["groupid"]
            ...
            if zb_node not in set(vcenter_vms.keys()):  # Node Needs to be deleted
                try:
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Error deleting node '{zb_node}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
        ...
        for vc_pon in set(vcenter_pon_vms.keys()):
                ...
                try:
                    ...
                    if (vcenter_pon_vms[vc_pon][0].startswith('vr1-') or vcenter_pon_vms[vc_pon][0].startswith('vr2-') or vcenter_pon_vms[vc_pon][0].startswith('vr3-')) and vcenter_pon_vms[vc_pon][7] != '1':  # Excluding not monitored
                        ...
                        if vcenter_pon_vms[vc_pon][6] == '1' and vcenter_pon_vms[vc_pon][1] != 'No_URL':  # Distinguishing VIPs
                            ...
                            if vcenter_pon_vms[vc_pon][0].startswith('vr1-'):
                                new_pon_host = zapi.host.create({
                                    ...
                                    "groups": [{"groupid": host_group_vr1_id}, {"groupid": host_group_support_id}, {"groupid": host_group_vrvip_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                            elif vcenter_pon_vms[vc_pon][0].startswith('vr2-'):
                                new_pon_host = zapi.host.create({
                                    "host": host_name,  # Technical name of the host (internal name)
                                    "name": host_visible_name,  # Visible name of the host in the frontend
                                    "description": host_desc,  # Host description
                                    "groups": [{"groupid": host_group_vr2_id}, {"groupid": host_group_support_id}, {"groupid": host_group_vrvip_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                            elif vcenter_pon_vms[vc_pon][0].startswith('vr3-'):
                                new_pon_host = zapi.host.create({
                                    "host": host_name,  # Technical name of the host (internal name)
                                    "name": host_visible_name,  # Visible name of the host in the frontend
                                    "description": host_desc,  # Host description
                                    "groups": [{"groupid": host_group_vr3_id}, {"groupid": host_group_support_id}, {"groupid": host_group_vrvip_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                        ...
                        if vcenter_pon_vms[vc_pon][6] == '0' and vcenter_pon_vms[vc_pon][1] != 'No_URL':  # Distinguishing Non VIPs
                            # Host information
                            host_name = vcenter_pon_vms[vc_pon][0]  # {HOST.HOST}
                            host_visible_name = vcenter_pon_vms[vc_pon][0]  # {HOST.NAME}
                            host_desc = f"Persian Name: {vcenter_pon_vms[vc_pon][3]}, National ID: {vcenter_pon_vms[vc_pon][5]}, Public IP: {vcenter_pon_vms[vc_pon][4]}"  # Host Description
                            ...
                            if vcenter_pon_vms[vc_pon][0].startswith('vr1-'):
                                new_pon_host = zapi.host.create({
                                    ...
                                    "name": host_visible_name,  # Visible name of the host in the frontend
                                    "description": host_desc,  # Host description
                                    "groups": [{"groupid": host_group_vr1_id}, {"groupid": host_group_support_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                                ...
                                new_pon_host = zapi.host.create({
                                    ...
                                    "name": host_visible_name,  # Visible name of the host in the frontend
                                    "description": host_desc,  # Host description
                                    "groups": [{"groupid": host_group_vr2_id}, {"groupid": host_group_support_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                                ...
                                new_pon_host = zapi.host.create({
                                    ...
                                    "name": host_visible_name,  # Visible name of the host in the frontend
                                    "description": host_desc,  # Host description
                                    "groups": [{"groupid": host_group_vr3_id}, {"groupid": host_group_support_id}, {"groupid": host_group_onprem_id}],  # Host group
                                    "templates": [{"templateid": template_vr_id}],  # Template
                                    "tags": tags,  # Tags
                                    "macros": macros
                                })
                    ...
                    if vcenter_pon_vms[vc_pon][0].startswith('vat-') and vcenter_pon_vms[vc_pon][7] != '1':  # Excluding not monitored
                        # Host information
                        host_name = vcenter_pon_vms[vc_pon][0]  # {HOST.HOST}
                        host_visible_name = vcenter_pon_vms[vc_pon][0]  # {HOST.NAME}
                        host_desc = f"Persian Name: {vcenter_pon_vms[vc_pon][3]}, National ID: {vcenter_pon_vms[vc_pon][5]}, Public IP: {vcenter_pon_vms[vc_pon][4]}"  # Host Description
                        ...
                        new_pon_host = zapi.host.create({
                            ...
                            "name": host_visible_name,  # Visible name of the host in the frontend
                            "description": host_desc,  # Host description
                            "groups": [{"groupid": host_group_vat_id}, {"groupid": host_group_support_id}],
                            # Host group
                            "templates": [{"templateid": template_vat_id}],  # Template
                            "tags": tags,  # Tags
                            "macros": macros
                        })
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Error adding node '{vc_pon}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
            ...
            if vc_pon not in set(vcenter_in_debt_vms.keys()):
                if vc_pon in set(zabbix_disabled_vms.keys()) and vcenter_pon_vms[vc_pon][7] != '1': # Excluding not monitored
                    try:
                        # Host information
                        ...
                        send_anonymous_email(
                            ...
                            to_email=error_receivers,
                            cc_email=default_cc,
                            subject=f"Error enabling node '{vc_pon}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
            ...
            if vc_poff in set(zabbix_enabled_vms.keys()):  # Node Needs to be disabled
                try:
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Error disabling node '{vc_poff}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
            ...
            if vc_disconnect in set(zabbix_enabled_vms.keys()):  # Node Needs to be disabled
                try:
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Error disabling node '{vc_disconnect}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
            ...
            if (vc_not_monitored in set(zabbix_enabled_vms.keys()) and vcenter_pon_vms[vc_not_monitored][7] == '1'):  # Node Needs to be disabled
                try:
                    ...
                    send_anonymous_email(
                        ...
                        to_email=error_receivers,
                        cc_email=default_cc,
                        subject=f"Error disabling node '{vc_not_monitored}' | Zabbix_OnPrem_VNK_Fully_Automated.py",
        ...
        if len(zabbix_nodes_added) > 0:
            ...
            send_anonymous_email(
                ...
                cc_email=default_cc,
                subject=f"{len(zabbix_nodes_added)} nodes added to ME-CustomerZabbix",
                html_message=f"Below nodes were added to ME-CustomerZabbix automatically:<br><br><b>{temp}</b>Agent: {script_name}",
                direction="ltr",
                # attachments=[]
            )
        ...
        if len(zabbix_nodes_enabled) > 0:
            ...
            send_anonymous_email(
                ...
                to_email=default_receivers,
                cc_email=default_cc,
                subject=f"{len(zabbix_nodes_enabled)} nodes got enabled in ME-CustomerZabbix",
                html_message=f"Below nodes were enabled again in ME-CustomerZabbix automatically:<br><br><b>{temp}</b>Agent: {script_name}",
                direction="ltr",
                # attachments=[]
            )
        ...
        if len(zabbix_nodes_disabled) > 0:
            ...
            send_anonymous_email(
                ...
                to_email=default_receivers,
                cc_email=default_cc,
                subject=f"{len(zabbix_nodes_disabled)} nodes got disabled in ME-CustomerZabbix",
                html_message=f"Below nodes were disabled in ME-CustomerZabbix automatically:<br><br><b>{temp}</b>Agent: {script_name}",
                direction="ltr",
                # attachments=[]
            )
        ...
        if len(zabbix_nodes_deleted) > 0:
            ...
            send_anonymous_email(
                ...
                to_email=default_receivers,
                cc_email=default_cc,
                subject=f"{len(zabbix_nodes_deleted)} nodes got deleted from ME-CustomerZabbix",
                html_message=f"Below nodes were deleted from ME-CustomerZabbix automatically:<br><br><b>{temp}</b>Agent: {script_name}",
                direction="ltr",
                # attachments=[]
            )
        ...
        if len(zabbix_nodes_updated) > 0:
            ...
            send_anonymous_email(
                ...
                to_email=default_receivers,
                cc_email=default_cc,
                subject=f"{len(zabbix_nodes_updated)} nodes got updated from ME-CustomerZabbix",
                html_message=f"Below nodes were updated in ME-CustomerZabbix automatically:<br><br><b>{temp}</b>Agent: {script_name}",
                direction="ltr",
                # attachments=[]
            )
