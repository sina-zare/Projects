...
# Create log-object
LOG_FILENAME = "/var/log/zabbix/unity_state.log"
...
def send_data_to_zabbix(zabbix_data, storage_name):
       ...
       # print('\n\nInitiating sina_sender...')
       # sina_sender = subprocess.run([sender_command, "-v", "-z", "127.0.0.1", "-p", "10051", "-s", storage_name, "-T", "-i", temp_file], stdout = subprocess.PIPE, stderr = subprocess.PIPE, text=True)
       # print('sina_sender finished...')
       # if sina_sender.stdout:
       #     print(f'sina_sender output:\n{sina_sender.stdout}')
       # if sina_sender.stderr:
       #     print(f'\nsina_sender error:\n{sina_sender.stderr}')
       # if sina_sender:
       #     print(f'sina_sender return code:\n{sina_sender}\n\n')


       # print("\n\nInitiating Aliyuk")
        send_code = subprocess.run([sender_command, "-vv", "-c", config_path, "-s", storage_name, "-T", "-i", temp_file], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
       # print('Aliyuk finished...')
       # if sina_sender.stdout:
       #     print(f'Aliyuk output:\n{send_code.stdout}')
       # if sina_sender.stderr:
       #     print(f'\nAliyuk error:\n{send_code.stderr}')
       # if send_code:
       #     print((str(send_code).split(',')[9]).split('=')[1])

        os.remove(temp_file)
        return (str(send_code).split(',')[9]).split('=')[1]
