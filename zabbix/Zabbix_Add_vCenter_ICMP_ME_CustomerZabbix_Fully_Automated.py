...
# --- Configuration ---
script_name = 'zabbix_add_vcenter_icmp_me_customerzabbix_fully_automated'
...
try:
    ...
    def send_anonymous_email(from_email, to_email, cc_email, subject, html_message, direction,
        ...
        try:
            ...
            send_anonymous_email(zabbix_server, error_receivers, default_cc,
                                 f"email_sender Function Error in running {script_name}.py",
    ...
    try:
        ...
        prefixes = ("mei-")
        trash_prefixes = ["template", "malekzade", "mei-chikanik", "mei-grasad", "mei-graz", "mei-gnobel",
                          "mei-gfarvardin", "mei-gazadi", "mei-gsharifi", "mei-gsalim", "mei-sarv", "mei-virasys",
                          "mei-masghariyan", "mei-arabzadeh", "mei-datyar", "mei-delino", "mei-farsad",
                          "mei-zarrin-srv10", "mei-mohsen-mail",
                          "mei-zarrin-srv6", "mei-razilab", "mei-garad"
                          ]
        ...
        zabbix_active_agent_template_names = ["Linux by Zabbix agent active", "Windows by Zabbix agent active"]
        ...
        for zahost in zabbix_active_template_hosts:
            zabbix_active_linwin_template_vms[zahost['host'].lower()] = [zahost['host'].lower(), zahost['hostid'],
                                                                         zahost['status']]
            ...
            if vc_pon not in set(zabbix_enabled_vms.keys()) and vc_pon not in set(
                ...
                try:
                    ...
                    if host_name.lower().startswith('mer-') or host_name.lower().startswith('merd-'):
                        ...
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mer_zabbix_host_groups, me_onprem_zabbix_tags)
                    ...
                    # MES
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mes_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MEF
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mef_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MEA
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mea_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MEB
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               meb_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MESA
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mesa_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MEM
                        ...
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mem_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # MEI
                    elif host_name.lower().startswith('mei-'):
                        added_successfully = False
                        added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                               mei_zabbix_host_groups, me_onprem_zabbix_tags)
                        if added_successfully:
                            zabbix_nodes_added.append(host_name)
                    # ME_MGMT
                        ...
                        if host_name.lower() in set(zabbix_active_linwin_template_vms.keys()):
                        ...
                        elif host_name.lower().startswith('me-rahlock') or host_name.lower().startswith(
                                'me-seplock') or host_name.lower().startswith('me-buildlock'):
                            ...
                            added_successfully = False
                            added_successfully = zabbix_node_adder(vcenter_pon_vms[vc_pon], zabbix_templates,
                                                                   me_mgmt_zabbix_host_groups, me_mgmt_zabbix_tags)
                            if added_successfully:
                                zabbix_nodes_added.append(host_name)
        ...
        if len(zabbix_nodes_added) > 0:
            ...
            send_anonymous_email(zabbix_server, default_receivers, default_cc,
                                 ...
                                 f"Below ICMP nodes were added to ME-CustomerZabbix automatically:<br><br><b>{temp}</b> Agent: {script_name}.py",
                                 'ltr')
        ...
        if len(zabbix_nodes_enabled) > 0:
            ...
            send_anonymous_email(zabbix_server, default_receivers, default_cc,
                                 ...
                                 f"Below ICMP nodes were enabled again in ME-CustomerZabbix automatically:<br><br><b>{temp}</b> Agent: {script_name}.py",
                                 'ltr')
        ...
        if len(zabbix_nodes_disabled) > 0:
            ...
            send_anonymous_email(zabbix_server, default_receivers, default_cc,
                                 ...
                                 f"Below ICMP nodes were disabled in ME-CustomerZabbix automatically:<br><br><b>{temp}</b> Agent: {script_name}.py",
                                 'ltr')
        ...
        if len(zabbix_nodes_deleted) > 0:
            ...
            send_anonymous_email(zabbix_server, default_receivers, default_cc,
                                 ...
                                 f"Below ICMP nodes were deleted from ME-CustomerZabbix automatically:<br><br><b>{temp}</b> Agent: {script_name}.py",
                                 'ltr')
